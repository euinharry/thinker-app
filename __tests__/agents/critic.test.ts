/**
 * Critic Agent Tests
 *
 * Tests for CriticAgent: initialization, chat(), streamChat(), error handling.
 * Uses mocked AI provider to avoid real API calls.
 */

import { describe, it, expect, vi, beforeEach } from "vitest";

// ============================================================================
// Mock Setup
// ============================================================================

const mockChatFn = vi.fn();
const mockStreamChatFn = vi.fn();

vi.mock("@/lib/ai/mimo-provider", () => {
  class MockMimoProvider {
    name = "mimo";
    chat = mockChatFn;
    streamChat = mockStreamChatFn;

    constructor(_config?: any) {}
  }

  return {
    MimoProvider: MockMimoProvider,
  };
});

const originalEnv = process.env;

beforeEach(() => {
  vi.clearAllMocks();
  process.env = { ...originalEnv };
  process.env.MIMO_API_KEY = "test-api-key";
  process.env.MIMO_BASE_URL = "https://api.test.mimo.ai/v1";
});

// ============================================================================
// CriticAgent Tests
// ============================================================================

describe("CriticAgent", () => {
  describe("initialization", () => {
    it("creates an instance with default config", async () => {
      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();

      expect(critic).toBeDefined();
      expect(critic.agentType).toBe("critic");
    });

    it("returns correct agent info", async () => {
      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();
      const info = critic.getAgentInfo();

      expect(info.id).toBe("critic");
      expect(info.name).toBe("Critic");
      expect(info.avatar).toBe("🎯");
      expect(info.personality).toContain("challenger");
    });

    it("returns system prompt", async () => {
      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();
      const prompt = critic.getSystemPrompt();

      expect(prompt).toBeDefined();
      expect(prompt.length).toBeGreaterThan(0);
      expect(prompt).toContain("Critic");
      expect(prompt).toContain("quality");
    });

    it("accepts custom configuration", async () => {
      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent({
        model: "custom-model",
        temperature: 0.5,
        maxTokens: 2048,
      });

      expect(critic).toBeDefined();
      expect(critic.agentType).toBe("critic");
    });
  });

  describe("chat()", () => {
    it("throws error for empty message", async () => {
      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();

      await expect(critic.chat("")).rejects.toThrow("Message cannot be empty");
    });

    it("throws error for whitespace-only message", async () => {
      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();

      await expect(critic.chat("   ")).rejects.toThrow("Message cannot be empty");
    });

    it("calls provider with correct request structure", async () => {
      mockChatFn.mockResolvedValue({
        id: "test-id",
        model: "mimo-v2.5-pro",
        choices: [
          {
            message: { role: "assistant", content: "Test response" },
            finish_reason: "stop",
          },
        ],
        usage: { prompt_tokens: 10, completion_tokens: 5, total_tokens: 15 },
      });

      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();
      const response = await critic.chat("What are the risks?");

      expect(mockChatFn).toHaveBeenCalledTimes(1);
      expect(mockChatFn).toHaveBeenCalledWith(
        expect.objectContaining({
          messages: [{ role: "user", content: "What are the risks?" }],
          model: "mimo-v2.5-pro",
          system_prompt: expect.stringContaining("Critic"),
          agent_name: "Critic",
        })
      );
      expect(response).toBe("Test response");
    });

    it("returns response content from provider", async () => {
      const expectedResponse = "Here are the critical risks I've identified...";

      mockChatFn.mockResolvedValue({
        id: "test-id",
        model: "mimo-v2.5-pro",
        choices: [
          {
            message: { role: "assistant", content: expectedResponse },
            finish_reason: "stop",
          },
        ],
        usage: { prompt_tokens: 10, completion_tokens: 20, total_tokens: 30 },
      });

      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();
      const response = await critic.chat("Review this approach");

      expect(response).toBe(expectedResponse);
    });

    it("throws error when provider returns empty response", async () => {
      mockChatFn.mockResolvedValue({
        id: "test-id",
        model: "mimo-v2.5-pro",
        choices: [
          {
            message: { role: "assistant", content: "" },
            finish_reason: "stop",
          },
        ],
        usage: { prompt_tokens: 10, completion_tokens: 0, total_tokens: 10 },
      });

      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();

      await expect(critic.chat("Hello")).rejects.toThrow("Empty response from AI provider");
    });

    it("re-throws provider errors", async () => {
      const providerError = {
        code: "rate_limit",
        message: "Rate limit exceeded",
        retryable: true,
      };

      mockChatFn.mockRejectedValue(providerError);

      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();

      await expect(critic.chat("Hello")).rejects.toEqual(providerError);
    });

    it("wraps unexpected errors", async () => {
      mockChatFn.mockRejectedValue(new Error("Network error"));

      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();

      await expect(critic.chat("Hello")).rejects.toThrow("Critic agent failed: Network error");
    });
  });

  describe("streamChat()", () => {
    it("throws error for empty message", async () => {
      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();

      await expect(critic.streamChat("")).rejects.toThrow("Message cannot be empty");
    });

    it("calls provider streamChat with correct request", async () => {
      const mockStream = {
        id: "stream-id",
        model: "mimo-v2.5-pro",
        [Symbol.asyncIterator]: async function* () {
          yield { id: "chunk-1", model: "mimo-v2.5-pro", delta: { content: "Risk" }, finish_reason: null };
        },
      };

      mockStreamChatFn.mockResolvedValue(mockStream);

      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();
      const stream = await critic.streamChat("Critique this");

      expect(mockStreamChatFn).toHaveBeenCalledTimes(1);
      expect(stream).toBeDefined();
    });

    it("re-throws provider errors on stream", async () => {
      const providerError = {
        code: "timeout",
        message: "Request timed out",
        retryable: true,
      };

      mockStreamChatFn.mockRejectedValue(providerError);

      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();

      await expect(critic.streamChat("Hello")).rejects.toEqual(providerError);
    });

    it("wraps unexpected errors on stream", async () => {
      mockStreamChatFn.mockRejectedValue(new Error("Connection lost"));

      const { CriticAgent } = await import("@/lib/agents/critic");
      const critic = new CriticAgent();

      await expect(critic.streamChat("Hello")).rejects.toThrow(
        "Critic agent stream failed: Connection lost"
      );
    });
  });
});
